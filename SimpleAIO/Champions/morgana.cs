    ...
    internal class Morgana
    {
        ...
        private static void OnGameUpdate(EventArgs args)
        {
            ...
            private static void OnDraw(EventArgs args)
            {
                ...
                {
                    if (mainMenu["Draw"].GetValue<MenuBool>("qRange").Enabled) if (Q.IsReady()) CircleRender.Draw(PlayerPos, Q.Range, Color.Red);
                    if (mainMenu["Draw"].GetValue<MenuBool>("wRange").Enabled) if (W.IsReady()) CircleRender.Draw(PlayerPos, W.Range, Color.Silver, 1);
                    if (mainMenu["Draw"].GetValue<MenuBool>("eRange").Enabled) if (E.IsReady()) CircleRender.Draw(PlayerPos, E.Range, Color.Brown, 1);
                }
            }
        }
    }
