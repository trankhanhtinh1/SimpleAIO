...
namespace SimpleAIO.Champions
{
    internal class RekSai
    {
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            {
                if (mainMenu["Draw"].GetValue<MenuBool>("EburrowedRange").Enabled && IsBurrowed() && Eburrowed.IsReady()) CircleRender.Draw(PlayerPos, Eburrowed.Range, Color.Yellow, 1);
                if (mainMenu["Draw"].GetValue<MenuBool>("EUnburrowedRange").Enabled && !IsBurrowed() && EUnburrowed.IsReady()) CircleRender.Draw(PlayerPos, Eburrowed.Range, Color.DarkGreen, 1);
            }
            if (!mainMenu["Draw"].GetValue<MenuBool>("lista").Enabled)
            {
                if (mainMenu["Draw"].GetValue<MenuBool>("EburrowedRange").Enabled && IsBurrowed() && Eburrowed.IsReady()) CircleRender.Draw(PlayerPos, Eburrowed.Range, Color.Yellow, 1);
