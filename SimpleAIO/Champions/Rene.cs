...
namespace SimpleAIO.Champions
{
    internal class Renekton
    {
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            if (!mainMenu["Draw"].GetValue<MenuBool>("lista").Enabled)
            {
                if (mainMenu["Draw"].GetValue<MenuBool>("qRange").Enabled) CircleRender.Draw(PlayerPos, Q.Range, Color.Cyan, 1);
                if (mainMenu["Draw"].GetValue<MenuBool>("eRange").Enabled) CircleRender.Draw(PlayerPos, E.Range, Color.Yellow, 1);
            }
        }
