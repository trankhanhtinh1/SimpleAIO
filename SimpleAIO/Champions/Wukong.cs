...
namespace SimpleAIO.Champions
{
    internal class Wukong
    {
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            {
                if (mainMenu["Draw"].GetValue<MenuBool>("qRange").Enabled && Q.IsReady()) CircleRender.Draw(PlayerPos, Q.Range, Color.Cyan, 1);
                if (mainMenu["Draw"].GetValue<MenuBool>("wRange").Enabled && W.IsReady()) CircleRender.Draw(PlayerPos, W.Range, Color.Silver, 1);
                if (mainMenu["Draw"].GetValue<MenuBool>("eRange").Enabled && E.IsReady()) CircleRender.Draw(PlayerPos, E.Range, Color.Yellow, 1);
                if (mainMenu["Draw"].GetValue<MenuBool>("rRange").Enabled && R.IsReady()) CircleRender.Draw(PlayerPos, R.Range, Color.Green, 1);
            }
        }
    }
}