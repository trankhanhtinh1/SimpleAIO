...
namespace SimpleAIO.Champions
{
    internal class Mordekaiser{
        ...
        public static void OnGameLoad(){
            if(GameObjects.Player.CharacterName != "Mordekaiser") return;
        ...
        private static void OnDraw(EventArgs args){
            if(mainMenu["Draw"].GetValue<MenuBool>("lista").Enabled){
                if(mainMenu["Draw"].GetValue<MenuBool>("qRange").Enabled){
                    if(Q.IsReady()){
                        CircleRender.Draw(GameObjects.Player.Position, Q.Range,Color.Cyan,1);
                    }
                }
                if(mainMenu["Draw"].GetValue<MenuBool>("wRange").Enabled){
                    if(W.IsReady()){
                        CircleRender.Draw(GameObjects.Player.Position, W.Range,Color.Silver,1);
                    }
                }
                if(mainMenu["Draw"].GetValue<MenuBool>("eRange").Enabled){
                    if(E.IsReady()){
                        CircleRender.Draw(GameObjects.Player.Position, E.Range,Color.Yellow,1);
                    }
